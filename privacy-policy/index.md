...
**Note:** This policy reflects the current state with Firebase Analytics, Crashlytics, Cloud Messaging,
and optional account (Sign Up / Sign In via Supabase) enabled.

## Introduction

Office Reader ("we," "our," or "the app") is committed to protecting your privacy. This Privacy
Policy explains how we handle your information when you use our document viewing and PDF tools
application.
...
Office Reader is designed as an **offline-first, privacy-focused** application. We do NOT collect,
transmit, or store any **personal documents or files** on external servers. All your documents
remain stored locally on your device only.

**Analytics Data:** We collect anonymized, technical data through Firebase Analytics and Crashlytics
to improve app performance and stability. This data does NOT include any personal documents or
user-identifiable data.

**Account Information (Optional):** If you choose to create an account (Sign Up), we collect:
...
- **Password** — stored securely by Supabase (we never see your plain password)
- **App activity** — XP, streak, documents opened, ads watched, etc. (for leaderboard and gamification)

Account creation is **optional**. You can use the App without an account ("Continue without account").
Account data is stored on Supabase servers.
...
- ❌ Personal identification information — **unless you create an account** (then we collect email, display name, avatar)
- ❌ User-entered text or annotations (except optional notes/bookmarks, stored locally)
- ❌ Display advertisements (ads may be shown to FREE users; no tracking for ads)
- ❌ Require account registration for core features — account is **optional**

**What We DO Collect:**

- ✅ **If you create an account:** Email, display name, avatar (optional), app activity (XP, streak, stats) — via Supabase
- ✅ App usage analytics (screen views, feature interactions) - via Firebase Analytics — anonymized
...
3. **Internet Permission**
    - Purpose: Google Drive integration, crash reporting, analytics, in-app updates, and push
      notifications
...
- Data is protected by your device's built-in security mechanisms (file system permissions, device
  lock screen, PIN, biometric authentication, etc.)
...
- Documents will remain on your device if you uninstall the app, but app preferences and history
  will be lost
...
- **Account Data (Supabase)**: Retained until you request deletion. You can sign out anytime; contact
  us to permanently delete your account and associated data.
- **Local Document Data**: Retained on your device until you delete it. This data is never
  transmitted or stored on external servers.
- **Analytics Data**:
    - **Firebase Analytics**: Retained according to Google's data retention policies (typically up
      to 14 months). Data is automatically deleted after the retention period.
    - **Firebase Crashlytics**: Retained for crash analysis and debugging purposes. Crash reports
      are kept to help us identify and fix issues.
    - You can request deletion of your analytics data at any time by contacting us at
      thuthao.finai@gmail.com
...
**Account Data:** To delete your account and data stored on Supabase (email, profile, stats), sign out
and contact us at thuthao.finai@gmail.com. We will process your request within 30 days.
...
**Local Data:** All your documents and app data are stored locally on your device and can be deleted
by clearing app data or uninstalling the app. This data is never transmitted to external servers.

**Analytics Data:** If you want to request deletion of analytics data collected by Firebase
Analytics or Crashlytics, please contact us at thuthao.finai@gmail.com. We will process your request
within 30 days. Note that analytics data is anonymized and does not contain any personal or document
information.

## Children's Privacy

Office Reader does not knowingly collect information from children under 13. The app is designed for
general document viewing purposes.

## Changes to Privacy Policy

We may update this Privacy Policy periodically. Changes will be posted in this document with an
...
Office Reader uses **Firebase Analytics** and **Firebase Crashlytics** (provided by Google) to
collect anonymized technical data for app improvement purposes.
...
- **Data Usage**: Used only for improving app features, understanding user needs, and enhancing
  overall app experience
- **Data Sharing**: Data is processed by Google (Firebase service provider) but is NOT shared with
  other third parties
...
- ✅ **NO user identification** is collected — **unless you create an account** (then email, name, avatar are stored on Supabase)
...
- **Supabase**: Used for optional account (Sign Up / Sign In). Stores email, display name, avatar,
  and app activity (XP, streak, stats). Passwords are hashed. [Supabase Privacy](https://supabase.com/privacy)
- **Google Sign-In & Drive API**: Used for optional Google Drive integration. Authentication is
  handled by Google. We do not store your Google credentials.
- **Google ML Kit**: Used for on-device text recognition (OCR) in the document scanner. All
  processing happens locally.
- **Google UMP (User Messaging Platform)**: Used for GDPR consent management for users in the
...
**Important:** These libraries operate locally and do not transmit document data to external
servers. OCR processing happens entirely on your device.
